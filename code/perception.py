...
def color_thresh(img, rgb_thresh_above=(160, 160, 160), rgb_thresh_below = None):
    ...
    if rgb_thresh_above:
        thresh_above = ((img[:,:,0] > rgb_thresh_above[0])) \
                & ((img[:,:,1] > rgb_thresh_above[1])) \
                & ((img[:,:,2] > rgb_thresh_above[2]))
    # Index the array of zeros with the boolean array and set to 1.
    if rgb_thresh_below:
        thresh_below = ((img[:,:,0] < rgb_thresh_below[0])) \
                & ((img[:,:,1] < rgb_thresh_below[1])) \
                & ((img[:,:,2] < rgb_thresh_below[2]))
    
    if thresh_above is not None and thresh_below is not None:
        thresh = thresh_above * thresh_below
    elif thresh_above is not None:
        thresh = thresh_above
    else:
        thresh = thresh_below
...
def perception_step(Rover):
    ...
    threshed_obstacle = color_thresh(warped, rgb_thresh_above = None, rgb_thresh_below = (160, 160, 160))
    threshed_rock = color_thresh(warped, rgb_thresh_above=(110, 115, 0), rgb_thresh_below = (250, 250, 100))
    ...
    terrain_x_world, terrain_y_world = pix_to_world(xpix_terrain, ypix_terrain, xpos, ypos, yaw, world_size, scale)
